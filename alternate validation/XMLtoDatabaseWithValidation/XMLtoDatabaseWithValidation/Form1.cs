...
namespace XMLtoDatabaseWithValidation
{
    public partial class Form1 : Form
    {
        ...
        private void btnWriteXMLtoDatabase_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string strXSDfileSource = @"C:\Users\ken\Documents\Visual Studio 2010\Projects\XMLtoDatabaseWithValidation\XMLtoDatabaseWithValidation\books.xsd";
                string strXMLfileSource = @"C:\Users\ken\Documents\Visual Studio 2010\Projects\XMLtoDatabaseWithValidation\XMLtoDatabaseWithValidation\books.xml";
                XmlReaderSettings settings = new XmlReaderSettings();
                settings.Schemas.Add(null, strXSDfileSource);
                settings.ValidationType = ValidationType.Schema;
                settings.ValidationFlags |= XmlSchemaValidationFlags.ProcessInlineSchema;
                settings.ValidationFlags |= XmlSchemaValidationFlags.ReportValidationWarnings;
                settings.ValidationEventHandler += new ValidationEventHandler(ValidationCallBack);

                XmlReader reader1 = XmlReader.Create(strXMLfileSource, settings);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Caught a problem: " + ex.Message);
